...
from keras.layers import (Activation, Conv2D, Dense, GlobalAveragePooling2D, GlobalMaxPooling2D, Add, Concatenate,
                          Conv1D, Input, Multiply, Reshape)
...
def se_block(input_feature, ratio=16, name=""):  # SENet
...
def channel_block(input_feature, ratio=8, name=""):
    channel = input_feature._keras_shape[-1]
...
def cbam_block(cbam_feature, ratio=8, name=""):  # CBAM
    cbam_feature = channel_block(cbam_feature, ratio, name=name)
    cbam_feature = spatial_attention(cbam_feature, name=name)
    return cbam_feature


def eca_block(input_feature, b=1, gamma=2, name=""):  # ECA
    channel = input_feature._keras_shape[-1]
